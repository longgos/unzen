...
<head>
	...
	<link rel='stylesheet' media='all' href="${base}/static/default/css/per.css"/>
    <#include "/default/inc/include.ftl"/>
</head>
<body>
	<!-- header -->
	<#include "/default/inc/per_header.ftl"/>
...
<!-- footer -->
    ...
    <#include "/default/inc/bot_nav.ftl"/>
<!--<button type="button" class="btn btn-primary btn-lg" data-toggle="modal" data-target="#userMsg">
  Launch demo modal
</button>-->
<#include "/default/model/model.ftl">
</html>
</#macro>
...
<#macro pager url p spans>
    ...
    <ul class="pagination">
        ...
        <#if (pageNo > 1)>
            <li><a href="${cURL}${pageNo - 1}" pageNo="${pageNo - 1}" class="prev">${pageNo}上一页</a></li>
        <#else>
            <li class="disabled"><span>${pageNo}-${pageCount}上一页</span></li>
        </#if>
